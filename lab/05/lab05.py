...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a if distance(city_a, city_c) < distance(city_b, city_c) else city_b)
...
def count_palindromes(L):
    ...
    2
    """
    return len(list(filter(lambda s: s[::-1] == s, map(lambda s: s.lower(), L))))
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return list(filter(lambda p: p[1] >= lower and p[1] <= upper, map(lambda x: [x, fn(x)], seq)))
